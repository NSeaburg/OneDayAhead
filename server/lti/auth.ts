...
export async function ltiAuthMiddleware(req: LtiSession, res: Response, next: NextFunction) {
  // Check for development session context set by /dev route
  if ((req as any).session?.ltiContext?.isDevelopment) {
    // Use mock LTI context from development session
    const devContext = (req as any).session.ltiContext;
    req.lti = {
  ...
  if (process.env.NODE_ENV === 'development' && (req.path.startsWith('/dev') || req.path.startsWith('/api/'))) {
    ...
    req.lti = {
      claims: {
        ...
        'https://purl.imsglobal.org/spec/lti/claim/message_type': req.query?.message_type === 'deep_link' ? 'LtiDeepLinkingRequest' : 'LtiResourceLinkRequest',
        'https://purl.imsglobal.org/spec/lti-dl/claim/deep_linking_settings': req.query?.message_type === 'deep_link' ? {
  ...
  try {
    ...
    console.log('=== AUTH MIDDLEWARE: JWT VALIDATED ===');
    ...
    console.log('req.lti object created successfully');
